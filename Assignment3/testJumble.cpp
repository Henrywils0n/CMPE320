...
//https://github.com/jeremyroy/projects/blob/master/Assignments%20for%20programming%20courses/C%2B%2B%20(CMPE%20320)/Assignment%204/testJumble/testJumble/jumble.cpp
...
void testException(){
    // PLEASE NOTE: I would use cerr, but it runs in parrallel and makes my output all wonky, so I just used cout instead
    JumblePuzzle jp= new JumblePuzzle();
    try{
        cout<<"Should throw Exception for word length:"<<endl;
        JumblePuzzle jp5("HELLLLOOOOOOOO", "medium");
    } catch (BadJumbleException& e){
        cout<< e.what() <<endl<<endl;
    }
    try{
        cout<<"Should throw Exception for difficulty:"<<endl;
        JumblePuzzle jp6("HELLO", "XXXXXX");
    } catch (BadJumbleException& e){
        cout<< e.what() <<endl<< endl;
    }
    try{
        cout<<"Should throw Exception for invalid characters:"<<endl;
        JumblePuzzle jp6("#$%^&*", "medium");
    } catch (BadJumbleException& e){
        cout<< e.what() <<endl<< endl;
    }
...
void testDifficultyLength(){
    JumblePuzzle easyJumble("Hello", "easy");
    cout<< "size should be 10:" <<endl;
    cout<< easyJumble.getSize() <<endl <<endl;

    JumblePuzzle mediumJumble("Hello", "medium");
    cout<< "size should be 15:" <<endl;
    cout<< mediumJumble.getSize() <<endl<<endl;

    JumblePuzzle hardJumble("Hello", "hard");
    cout<< "size should be 20:" <<endl;
    cout<< hardJumble.getSize() <<endl<<endl;
}

void testAccessors(){
    JumblePuzzle jp("hello", "medium");
    int col = jp.getColPos();
    int row = jp.getRowPos();
    int size = jp.getSize();
    char dir = jp.getDirection();
    cout << "Should return valid column and row (look for test passed)" << endl;
    if (col < size && col>=0 && row>=0 && row<size){
        cout << "test passed" << endl << endl;
    }
    cout << "Should return valid direction (look for test passed)" << endl;
...
//    cout << "\nPassed memory leak test!" << endl << endl;
